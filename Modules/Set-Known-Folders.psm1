...
function Set-KnownFoldersToOneDrive {
    Param(
        [Parameter(Mandatory = $true)]
        [string]$PathToOneDrive
    )

    Write-Host "OneDrive Path: $PathToOneDrive"

    Set-KnownFolderPath -KnownFolder Contacts -Path "$PathToOneDrive\Contacts"
    Set-KnownFolderPath -KnownFolder Downloads -Path "$PathToOneDrive\Downloads"
    Set-KnownFolderPath -KnownFolder Documents -Path "$PathToOneDrive\Documents"
    Set-KnownFolderPath -KnownFolder Pictures -Path "$PathToOneDrive\Pictures"
    Set-KnownFolderPath -KnownFolder Videos -Path "$PathToOneDrive\Videos"
    Set-KnownFolderPath -KnownFolder Favorites -Path "$PathToOneDrive\Favorites"
}
...
function Set-KnownFolderPath {
    Param (
            [Parameter(Mandatory = $true)]
            [ValidateSet('AddNewPrograms', 'AdminTools', 'AppUpdates', 'CDBurning', 'ChangeRemovePrograms', 'CommonAdminTools', 
                'CommonOEMLinks', 'CommonPrograms', 'CommonStartMenu', 'CommonStartup', 'CommonTemplates', 'ComputerFolder', 
                'ConflictFolder', 'ConnectionsFolder', 'Contacts', 'ControlPanelFolder', 'Cookies', 'Desktop', 'Documents', 
                'Downloads', 'Favorites', 'Fonts', 'Games', 'GameTasks', 'History', 'InternetCache', 'InternetFolder', 
                'Links', 'LocalAppData', 'LocalAppDataLow', 'LocalizedResourcesDir', 'Music', 'NetHood', 'NetworkFolder', 
                'OriginalImages', 'PhotoAlbums', 'Pictures', 'Playlists', 'PrintersFolder', 'PrintHood', 'Profile', 
                'ProgramData', 'ProgramFiles', 'ProgramFilesX64', 'ProgramFilesX86', 'ProgramFilesCommon', 'ProgramFilesCommonX64', 
                'ProgramFilesCommonX86', 'Programs', 'Public', 'PublicDesktop', 'PublicDocuments', 'PublicDownloads', 
                'PublicGameTasks', 'PublicMusic', 'PublicPictures', 'PublicVideos', 'QuickLaunch', 'Recent', 'RecycleBinFolder', 
                'ResourceDir', 'RoamingAppData', 'SampleMusic', 'SamplePictures', 'SamplePlaylists', 'SampleVideos', 'SavedGames', 
                'SavedSearches', 'SEARCH_CSC', 'SEARCH_MAPI', 'SearchHome', 'SendTo', 'SidebarDefaultParts', 'SidebarParts', 
                'StartMenu', 'Startup', 'SyncManagerFolder', 'SyncResultsFolder', 'SyncSetupFolder', 'System', 'SystemX86', 
                'Templates', 'TreeProperties', 'UserProfiles', 'UsersFiles', 'Videos', 'Windows')]
    ...
    Write-Host "Known Folder: $KnownFolder"
...
'@
    ...
    if (-Not(Test-Path $Path -PathType Container)) {
        $parentFolderPath = Split-Path $Path
